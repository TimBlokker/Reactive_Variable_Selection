...
lm_regsubsets<-reactive({
  lm_regsubsets <- leaps::regsubsets(
     ...
     nvmax = min(10, length(colnames(matrices_list[[input$matrix]]))),
...
BIC is indeed more stringent then the other, also you see how RSS and R² are not valid selection criteriums since they strictly improve with additionaly predictors.


### Best model accodring to adjusted R²
```{r}

renderPrint({
  summary(lm_regsubsets())[["adjr2"]]
})

renderPrint({
max(summary(lm_regsubsets())[["adjr2"]])
})

renderPrint({
which(summary(lm_regsubsets())[["adjr2"]]==max(summary(lm_regsubsets())[["adjr2"]]))
})
```

```{r}
renderPrint({
x<-summary(lm_regsubsets())
...
```{r}
renderPlot({
plot(lm_regsubsets(), scale="adjr2")
})
```

```{r}
renderPlot({
plot(lm_regsubsets(), scale="bic")
})
```
