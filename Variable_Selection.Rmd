...
This is a relatively sparse model and I try to comapre it to the cv.glmnet function as before. I realized after a while that I am not getting an as good result because my lambda steps are not small enough but it gets very slow when I try to take very small steps. Maybe the algorithm for the caret package is more efficient? Or makes assumptions and might actually be less accurate.
...
cv.alpha<-reactive({
  ...
  foldid=sample(1:10,size=length(y),replace=TRUE) #TODO keeping foldid constant - to optimize alpha and lambda on same folds??? - check this
