...
standardize_pred<-function(df, response="Transition" ){
     ...
     df<-df%>%select(!starts_with(response))
...
save_response<-function(df, response){
  df%>%select(starts_with(response))
}
...
culDifClust<-reactive({
  req(input$matrix)
  d<-dist(1-abs(cor(matrices_list()[[input$matrix]]%>% select(!starts_with("Trans")))))
...
clusterd<-reactive({
  data.frame("Cluster_num"=culDif.gp(), abs(cor(matrices_list()[[input$matrix]]%>% select(!starts_with("Trans")))), "Predictors"=names(culDif.gp()))
})
...
#'
#' @param regsubset_obj object of class regsubset that is contains the models of different sizes
...
#' @return The matrix product of the model.matrix containing the selected variables (nxp) and the coefficient vector (px1) resulting in a nx1 vector with the values of the response variable for every 
...
mean_cv_errors<-reactive({
  ...
  predictors<-matrices_list()[[input$matrix]]%>%
    select(!starts_with("Transition"))
  data<-matrices_list()[[input$matrix]]%>%
    select(c(!starts_with("Transition"),matches(input$response)))
  ...
  for(test_fold in 1:k) {
    best.fit<-leaps::regsubsets(
      x=f,
      data=data%>%filter(folds!=test_fold),
      nvmax = dim(data)[2],
      method=input$step_method)
    
    for(number_of_vars in 1:dim(predictors)[2]){
      pred<-predict.regsubsets(best.fit, number_of_vars, data%>%filter(folds==test_fold),f)
      cv_errors[test_fold, number_of_vars ] <-MSE(data%>%select(input$response)%>%filter(folds==test_fold), pred)
    }
  }
...
<!--                         paste(colnames(matrices_list_x[["transition_distances_influenza_tt"]]%>%select(!starts_with("Trans"))), collapse = "+"))), -->
