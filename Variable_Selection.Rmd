...
culDifClust<-reactive({
  req(input$matrix)
  d<-dist(abs(cor(matrices_list[[input$matrix]]%>% select(!starts_with("Trans")))))
...
plot(dend,main = "Cluster Dendrogram", ylab="Average Link Distance", xlab="Predictors", )
})
```
...
clusterd<-reactive({
  data.frame("Cluster_num"=culDif.gp(), "Predictors"=names(culDif.gp()))
})
...
<!--   rpart=rpart(as.factor(clusterd()$V1)~., data=clusterd()) -->
